...
public class Optimize extends VisitorAdapter {
    ...
    public void visit(Method method) {
        if((method.accessFlag & Const.NATIVE) != 0 || (method.accessFlag & Const.ABSTRACT) != 0)
            return ;
        initLabelMap(method);
        this.pStart = Integer.parseInt(method.registers_directive_count);
        this.pStart -= method.prototype.argsType.size();
        if((method.accessFlag & Const.STATIC) == 0)
            this.pStart --;
        this.ip = 0;
    ...
    public void initLabelMap(Method method) {
        ...
        if(stmIndex < insts.size() && currentLabel.lab.startsWith("try_end_")) {
            this.labelMap.put(currentLabel.lab, stmIndex);
            labelIndex++;
            if (labelIndex < labelList.size()) {
                currentLabel = labelList.get(labelIndex);
