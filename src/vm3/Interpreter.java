...
import static vm3.Util.hex2int;
import static vm3.Util.hex2long;
...
//        Util.printErr("Unimplemented instruction: " + inst.op);
        vm.reg[inst.vdest] = inst.type; // Load the Class here or not ?
        vm.pc++;
	}
	...
	public void visit(Instruction.NewArray inst) {
        vm.reg[inst.vdest] = new Array(vm, inst.type, (Integer)vm.reg[inst.vsize]);
		vm.pc++;
	}
    ...
    private boolean intTest(Object obj1, Object obj2, String cmp){
        ...
        if (obj1 instanceof Instance){
            ...
            switch (cmp){
                case "eq": return obj1 == null;
                case "ne": return obj1 != null;
                default:
        ...
        assert !(obj1 instanceof Instance);
        int val1 = obj1 == null ? 0 : (Integer) obj1;
        int val2 = obj2 == null ? 0 : (Integer) obj2;
        switch (cmp){
	...
	private void aget(int dstReg, int arrReg, int indexReg) {
		vm.reg[dstReg] = ((Array) vm.reg[arrReg]).aget((Integer)vm.reg[indexReg]);
		vm.pc++;
	}

	private void aput(int srcReg, int arrReg, int indexReg) {
		((Array) vm.reg[arrReg]).aput((Integer)vm.reg[indexReg], vm.reg[srcReg]);
		vm.pc++;
	}

	@Override
	public void visit(Instruction.Aget inst) {
		aget(inst.vdest, inst.varray, inst.vindex);
	}
	...
	public void visit(Instruction.Aput inst) {
		aput(inst.vsrc, inst.varray, inst.vindex);
	}
...
//		}
        ...
        if (vm.reg[inst.argvs[0]] instanceof String){
            new VMException(vm).printStackTrace();
            // const-class && reflection
            String Klass = "Ljava/lang/Class;";
            assert inst.type.classType.equals(Klass);
            vm.setExecuteEnv(Klass, vm.getMethod(Klass, inst.type.getMethodSign()), inst.argvs);
        }else{
            String objType = ((Instance)vm.reg[inst.argvs[0]]).clazzName;
            vm.setExecuteEnv(objType, vm.getMethod(objType, inst.type.getMethodSign()), inst.argvs);
        }
	}
	...
	public void visit(Instruction.InvokeSuper inst) {
        ...
        vm.setExecuteEnv(superType, vm.getMethod(superType, inst.type.getMethodSign()), inst.argvs);
	}
	...
	public void visit(Instruction.InvokeDirect inst) {
        ...
        if((inst.type.classType + "->" + inst.type.getMethodSign()).equals("Ljava/lang/Object;-><init>()V")){
            ...
            vm.setExecuteEnv(clazzName, vm.getMethod(clazzName, inst.type.getMethodSign()), inst.argvs);
        }
	}
	...
	public void visit(Instruction.InvokeStatic inst) {
        ...
        if (m.isNative){
            ...
            // filter
            switch ((inst.type.classType + "->" + inst.type.getMethodSign())){
                case "Ldalvik/system/VMStack;->getCallingClassLoader()Ljava/lang/ClassLoader;":
                    vm.returnValue = null;
                    vm.pc++;
                    break;
                case "Ljava/lang/VMThread;->currentThread()Ljava/lang/Thread;":
                    vm.returnValue = null; // I read the Java source code
                    vm.pc++;
                    break;
                case "Ljava/lang/System;->arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V":
                    assert vm.reg[(Integer)inst.argvs[0]] instanceof Array;
                    assert vm.reg[(Integer)inst.argvs[2]] instanceof Array;
                    Array arr1 = (Array)vm.reg[(Integer)inst.argvs[0]];
                    int arr1start = (Integer)vm.reg[(Integer)inst.argvs[1]];
                    Array arr2 = (Array)vm.reg[(Integer)inst.argvs[2]];
                    int arr2start = (Integer)vm.reg[(Integer)inst.argvs[3]];
                    int len = (Integer)vm.reg[(Integer)inst.argvs[4]];
                    Array.arraycopy(arr1, arr1start, arr2, arr2start, len);
                    vm.pc++;
                    break;
                default:
                    new VMException(vm).printStackTrace();
                    Util.printErr("Ooops! native");
//                try {
//                    String clazzName = inst.type.classType;
