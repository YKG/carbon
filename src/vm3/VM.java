...
{
  /**
   * code: simulate x86's CS register pc: simulate x86's IP register, '-1'
   * indicate the program finished, 'code[pc]' is the current instruction.
   */
  Object[] reg;
  ast.stm.T[] code;
  int pc;
  ...
  public void run()
  {
    ...
    pc = -2;
    saveThreadState();
    setExecuteEnv(mainClazzName,
        getMethod(mainClazzName, "main([Ljava/lang/String;)V"), new int[0]);

    while (pc != -1) {
      if (counter == 297) {
        counter = counter;
      }
      Debug.info("[" + currentClazz + "->" + currentMethod + "] [pc: " + pc
          + "]" + code[pc] + " " + counter++);
